...
bool tmessage, tfile;
char temphomedir[4096];
snprintf(temphomedir, 4096, "%s", getenv("HOME"));
while (fgets(input, 128, fp) != NULL){
    sscanf(input, "%lld %s %s %d %d:%d:%d %d", &entry[i].number, entry[i].weekday, entry[i].month, &entry[i].date, &entry[i].hour, &entry[i].minute, &entry[i].second, &entry[i].year);
    sprintf(temp, "%02d/%02d/%04d %02d:%02d", entry[i].date, monthdecode(entry[i].month)+1, entry[i].year, entry[i].hour, entry[i].minute);
    sprintf(tempfilename, "%s/PAS/pannouncements/%02d_%02d_%04d_%02d_%02d.pa", temphomedir, entry[i].date, monthdecode(entry[i].month), entry[i].year, entry[i].hour, entry[i].minute);
